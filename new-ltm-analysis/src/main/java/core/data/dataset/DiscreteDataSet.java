...
import core.variable.DiscreteVariable;
...
public final class DiscreteDataSet {
	...
	public DiscreteDataSet(Instances data) {
		...
		for(int i=0; i<num_variables; i++)
		{
			...
			variables.add(new DiscreteVariable(att.name(), states));
		}
	...
	// TODO: Permitir la transformación entre un DiscreteDataSet(discreto) y Data
...
//		_count++;
	 ...
	 */
	public DiscreteVariable[] getDiscreteVariables() {
		return _variables;
	}
...
//				+ ((System.currentTimeMillis())));
	 ...
	 */
	public Map<DiscreteVariable, Integer> createDiscreteVariableToIndexMap() {
		return Algorithm.createIndexMap(_variables);
	}
	
	public DiscreteDataSet SampleWithOverSampling(DiscreteVariable Lable) {
		DiscreteDataSet AfterSample = new DiscreteDataSet(getDiscreteVariables());
		...
		for(int i=0; i < Car; i++)
		{
			...
			{
				int LableIndex = Arrays.binarySearch(getDiscreteVariables(), Lable);
