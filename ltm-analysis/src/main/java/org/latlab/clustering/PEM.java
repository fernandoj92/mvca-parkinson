...
//			_workingData = doHardAssignmentParallelly(CurrentModel, Alayer);
	 ...
	 * model.getVariables()) { if(!model.getNode(var).isRoot()){ Matrix
	 * cptMat=FunToMat(model.getNode(var));
	 * if(cptMat.max()>0.999||cptMat.min()<0.001){ for(int i=0; i<cptMat.rows();
	 * i++) { for(int j=0;j<cptMat.columns();j++){ cptMat.set(i, j,
	 * (cptMat.get(i, j)+Math.random()*0.01)); } } cptMat =
	 * Normalize_Col(cptMat); setVdis(var,cptMat,model); } } } return model; }
	 */
	...
	private LTM reorderStates(LTM bn, HashMap<Integer, HashSet<String>> varDiffLevels) {
		...
		for (Variable latent : bn.getInternalVars("tree")) {
			...
			List<Entry<Variable, Double>> list = SortChildren(latent, setNode, ctp);
	...
	private List<Entry<Variable, Double>> SortChildren(Variable var, Set<DirectedNode> nodeSet,
		...
		List<Entry<Variable, Double>> List = Utils.sortByDescendingOrder(children_mi);
	...
	private DataSet doHardAssignmentParallelly(LTM CurrentModel, LTM latentTree) {
        ArrayList<DataCase> data = _Origdata.getData();

        Variable[] variables = new Variable[latentTree.getInternalVars().size()];

        int index = 0;
        for (Variable latent : latentTree.getInternalVars()) {
            Variable clone =
                    CurrentModel.getNodeByName(latent.getName()).getVariable();
            variables[index] = new Variable(clone.getName(), clone.getStates());
            index++;
        }
        
        ParallelHardAssignment pha = new ParallelHardAssignment(
        		_Origdata, CurrentModel, variables);
        pha.compute();

        DataSet da = new DataSet(variables);
        for (int j = 0; j < data.size(); j++) {
            da.addDataCase(pha.values.get(j), data.get(j).getWeight());
        }

        return da;
    }

	private static class ParallelHardAssignment extends RecursiveAction {
		private static final long serialVersionUID = 1L;
		private final DataSet data;
		private final LTM currentModel;
		private final Variable[] variables;
		private final List<int[]> values;
		private final int start;
		private final int length;
		private final ThreadLocal<CliqueTreePropagation> localCtp;
		private static final int THRESHOLD = 100;

		public ParallelHardAssignment(int start, int length, DataSet data,
                                      LTM currentModel, Variable[] variables,
                                      ThreadLocal<CliqueTreePropagation> localCtp) {
			this.data = data;
			this.start = start;
			this.length = length;
			this.currentModel = currentModel;
			this.variables = variables;
			this.localCtp = localCtp;
			values = new ArrayList<int[]>(length);
		}

		public ParallelHardAssignment(DataSet data, final LTM currentModel,
                                      Variable[] variables) {
			this(0, data.getData().size(), data, currentModel, variables,
					new ThreadLocal<CliqueTreePropagation>() {
						@Override
						protected CliqueTreePropagation initialValue() {
							return new CliqueTreePropagation(currentModel);
						}
					});
		}
		...
		protected void compute() {
			...
			int split = length / 2;
			ParallelHardAssignment c1 = new ParallelHardAssignment(start, split, data,
					currentModel, variables, localCtp);
			ParallelHardAssignment c2 = new ParallelHardAssignment(start + split,
					length - split, data, currentModel, variables, localCtp);
			invokeAll(c1, c2);

			values.addAll(c1.values);
			values.addAll(c2.values);
		}

		private void computeDirectly() {
			CliqueTreePropagation ctp = localCtp.get();
			int[] newValues = new int[variables.length];
			// update for every data case
			for (int j = 0; j < start + length; j++) {
				DataCase dataCase = data.getData().get(j);
				...
				for (int i = 0; i < variables.length; i++) {
					Variable latent = ((BeliefNode) currentModel
							.getNode(variables[i].getName())).getVariable();
