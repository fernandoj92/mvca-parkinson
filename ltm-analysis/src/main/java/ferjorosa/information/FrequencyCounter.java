...
public class FrequencyCounter {
    ...
    public ArrayList<double[]> computeParallel(DataSet data, List<Variable> variables) {
    ...
    // Metodo interfaz, es decir, el que se llama or parte del usuario
    public ArrayList<double[]> computeSequential(DataSet data, List<Variable> variables) {
    ...
    public ArrayList<double[]> computeFrequencies(DataSet data, List<Variable> variables, int start, int end) {
    ...
    private class ParallelComputation extends RecursiveAction {
        ...
        private DataSet data; // Esto seria una val en Scala (final en java solo sirve para las referencias, deberia ser immutable tmb)
        private List<Variable> variables; // Esto seria una val en Scala (final en java solo sirve para las referencias, deberia ser immutable tmb)
        private static final int THRESHOLD = 500;
        private ArrayList<double[]> frequencies;

        private ParallelComputation(DataSet data, List<Variable> variables, int start, int end) {
            this.start = start;
            this.end = end;
            this.data = data;
