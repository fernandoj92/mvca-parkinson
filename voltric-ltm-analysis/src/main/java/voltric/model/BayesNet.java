...
public class BayesNet extends DirectedAcyclicGraph {
	 ...
	 */
	protected HashMap<DiscreteVariable, BeliefNode> _variables;
	 ...
	 * BN change.
	 */
	protected HashMap<DiscreteDataSet, Double> _loglikelihoods;
	...
	public BayesNet(String name) {
		...
		_loglikelihoods = new HashMap<DiscreteDataSet, Double>();
	 ...
	 */
	public final BeliefNode addNode(DiscreteVariable variable) {
	...
	public BayesNet clone() {
		...
		copy._loglikelihoods = (HashMap<DiscreteDataSet, Double>) _loglikelihoods
				.clone();
	 ...
	 */
	public final double getAICcScore(DiscreteDataSet data) {
	 ...
	 */
	public final double getAICScore(DiscreteDataSet data) {
		// TODO We will deal with the expiring case later.
		double logL = this.getLoglikelihood(data);
	 ...
	 */
	public final double getBICScore(DiscreteDataSet data) {
	 ...
	 * @return
	 */
	public final Set<DiscreteVariable> getInternalVars(String input) {
		Set<DiscreteVariable> vars = new HashSet<DiscreteVariable>();
		BayesNet model = clone();
     ...
     * @return
     */
	public final Set<DiscreteVariable> getLeafVars(String input) {
		Set<DiscreteVariable> vars = new HashSet<DiscreteVariable>();
		BayesNet model = clone();
	 ...
	 */
	public final double getLoglikelihood(DiscreteDataSet dataSet) {
	 ...
	 */
	public final BeliefNode getNode(DiscreteVariable variable) {
	 ...
	 */
	public final Set<DiscreteVariable> getVariables() {

		Set<DiscreteVariable> vars = new HashSet<DiscreteVariable>();
		for (DiscreteVariable var : _variables.keySet()) {
			vars.add(var);
		}
		return vars;
	 ...
	 * @return
	 */
	public BayesNet manipulation_VariableReplacement(DiscreteVariable latVar, DiscreteVariable newVar) {
	 ...
	 */
	public DiscreteDataSet sample(int sampleSize) {
		// initialize data set
		int nNodes = getNumberOfNodes();
		DiscreteDataSet samples = new DiscreteDataSet(getVariables().toArray(
	...
	//added by peixian
		...
		for (AbstractNode node : getNodes()) {
			DiscreteVariable var = ((BeliefNode) node).getVariable();
		...
		for (int i = 0; i < sampleSize; i++) {
			...
			for (AbstractNode node : order) {
				...
				ArrayList<DiscreteVariable> parents = new ArrayList<DiscreteVariable>();
				ArrayList<Integer> parentStates = new ArrayList<Integer>();
				for (DirectedNode parent : bNode.getParents()) {
					DiscreteVariable var = ((BeliefNode) parent).getVariable();
					parents.add(var);
...
//		PrintWriter out = new PrintWriter(file);
		...
		// outputs nodes
		for (AbstractNode node : _nodes) {
			DiscreteVariable variable = ((BeliefNode) node).getVariable();
