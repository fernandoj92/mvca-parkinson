...
import voltric.util.TrialVersionLimitException;
...
public class AnalysisEngine {
	 ...
	 */
	private Map<Set<DiscreteVariable>, AnalysisResult> _analysisResults;
	 ...
	 * results.
	 */
	private List<Set<DiscreteVariable>> _analyzedLatents;
	 ...
	 */
	private DiscreteDataSet _samples;
	 ...
	 */
	public void analyze(Collection<DiscreteVariable> latents) {
		// only executes for latent variable set that has not be analyzed yet
		if (!hasAnalyzed(latents)) {
			// initialize data structure
			Set<DiscreteVariable> latentSet = new HashSet<DiscreteVariable>(latents);
	...
	public void analyzeAllIndividualLatents() {
		...
		for (DiscreteVariable latent : _model.getInternalVars()) {
			Collection<DiscreteVariable> latents = new HashSet<DiscreteVariable>();
			latents.add(latent);
			analyze(latents);
		}
	}
	 ...
	 *            computed.
	 */
	private void computeCcpds(Collection<DiscreteVariable> latents) {
		AnalysisResult result = getAnalysisResult(latents);

		// a function for relating latent states and their indices
		Function index = Function.createFunction(new ArrayList<DiscreteVariable>(
				latents));

		// latent variable array and their states
		int nLatents = index.getDimension();
		DiscreteVariable[] latentArray = index.getVariables().toArray(
				new DiscreteVariable[nLatents]);
		int[] states = new int[nLatents];

		// consider each latent state
		for (int i = 0; i < index.getDomainSize(); i++) {
			Map<DiscreteVariable, Function> ccpd = new HashMap<DiscreteVariable, Function>();
			...
			for (DiscreteVariable manifest : _model.getLeafVars()) {
	 ...
	 */
	public void computeCmiSeq(Collection<DiscreteVariable> latents, double maxCmiPercent) {
		AnalysisResult result = getAnalysisResult(latents);

		List<DiscreteVariable> order = result._order;
		...
		if (maxCmiPercent > result._largestMaxCmiPercent) {
			for (int i = cmiSeq.size(); i < order.size(); i++) {
				...
				List<DiscreteVariable> manifests = order.subList(0, i + 1);
	 ...
	 *            to be computed.
	 */
	public void computeCmiSeq(Collection<DiscreteVariable> latents,
			List<DiscreteVariable> manifests) {
		AnalysisResult result = getAnalysisResult(latents);
	 ...
	 * @return The MI I(x; y).
	 */
	private double computeMi(Collection<DiscreteVariable> x, Collection<DiscreteVariable> y) {
		// exact computation in case of |x| = 1 and |y| = 1
		if (x.size() == 1 && y.size() == 1) {
			List<DiscreteVariable> xyNodes = new ArrayList<DiscreteVariable>();
		...
		// xy = x union y
		ArrayList<DiscreteVariable> xy = new ArrayList<DiscreteVariable>();
		xy.addAll(x);
		xy.addAll(y);

		// samples over xy
		DiscreteDataSet xySamples = _samples.project(xy);

		// array of xy
		DiscreteVariable[] xyArray = xySamples.getVariables();
		int xySize = xyArray.length;

		// locate x and y in xyArray, respectively
		List<Integer> xPos = new ArrayList<Integer>();
		for (DiscreteVariable var : x) {
			xPos.add(Arrays.binarySearch(xyArray, var));
		}

		List<Integer> yPos = new ArrayList<Integer>();
		for (DiscreteVariable var : y) {
		...
		for (DiscreteDataCase d : xySamples.getData()) {
			// compute P(dx, dy)
			...
			for (int pos : yPos) {
				states[pos] = DiscreteDataSet.MISSING_VALUE; // hide values of y
			}
			...
			System.arraycopy(d.getStates(), 0, states, 0, xySize);
			for (int pos : xPos) {
				states[pos] = DiscreteDataSet.MISSING_VALUE; // hide values of x
			}
			_posteriorCtp.setEvidence(xyArray, states);
	 ...
	 */
	private void computeNaturalOrder(Collection<DiscreteVariable> latents) {
		final AnalysisResult result = getAnalysisResult(latents);

		// sort all manifest variables
		result._order.addAll(_model.getLeafVars());

		Collections.sort(result._order, new Comparator<DiscreteVariable>() {

			public int compare(DiscreteVariable var1, DiscreteVariable var2) {
	 ...
	 */
	private void computePmi(Collection<DiscreteVariable> latents) {
		AnalysisResult result = getAnalysisResult(latents);

		List<DiscreteVariable> manifests = new ArrayList<DiscreteVariable>();
		manifests.add(null);
	 ...
	 *            computed.
	 */
	private void computePrior(Collection<DiscreteVariable> latents) {
		// find the corresponding belief nodes
		Collection<DiscreteVariable> nodes = new ArrayList<DiscreteVariable>();
	 ...
	 */
	private void computeTmi(Collection<DiscreteVariable> latents) {
		AnalysisResult result = getAnalysisResult(latents);
	 ...
	 */
	public AnalysisResult getAnalysisResult(Collection<DiscreteVariable> latents) {
		int index = _analyzedLatents.indexOf(new HashSet<DiscreteVariable>(latents));
		if (index < 0) {
			return null;
		} else {
	 ...
	 */
	public List<Set<DiscreteVariable>> getAnalyzedLatents() {
	 ...
	 *         set has been analyzed.
	 */
	public boolean hasAnalyzed(Collection<DiscreteVariable> latents) {
		int index = _analyzedLatents.indexOf(new HashSet<DiscreteVariable>(latents));
		if (index < 0) {
			return false;
		} else {
	...
	private void initialize(boolean toSample) {
		...
		_analysisResults = new HashMap<Set<DiscreteVariable>, AnalysisResult>();
		_analyzedLatents = new ArrayList<Set<DiscreteVariable>>();
	...
	public void load(String path) throws IOException {
		...
		// load samples
		_samples = new DiscreteDataSet(path + File.separator + "samples");
		...
		for (String filename : dir.list()) {
			...
			Set<DiscreteVariable> latents = result.getLatents();
	 ...
	 *            be removed.
	 */
	public void removeAnalysisResult(Collection<DiscreteVariable> latents) {
		int index = _analyzedLatents.indexOf(new HashSet<DiscreteVariable>(latents));
		if (index >= 0) {
	...
	public void save(String path) throws FileNotFoundException,
		...
		for (Set<DiscreteVariable> latents : _analyzedLatents) {
			// file name
