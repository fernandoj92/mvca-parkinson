...
public class AnalysisResult extends CmiTree {
	...
	static AnalysisResult load(String filename, BayesNet model)
		...
		while (line.startsWith("% CCPDs")) {
			Map<DiscreteVariable, Function> ccpds = new HashMap<DiscreteVariable, Function>();

			while (!(line = in.readLine()).startsWith("%")) {
				tokens = line.split(" ");
				DiscreteVariable manifest = model.getNodeByName(tokens[0])
						.getVariable();
		...
		tokens = in.readLine().split(" ");
		ArrayList<DiscreteVariable> latents = new ArrayList<DiscreteVariable>();
		for (String token : tokens) {
	 ...
	 */
	List<Map<DiscreteVariable, Function>> _ccpds;
	 ...
	 */
	List<DiscreteVariable> _selectedManifests;
	...
	AnalysisResult() {
		...
		_ccpds = new ArrayList<Map<DiscreteVariable, Function>>();
		_order = new ArrayList<DiscreteVariable>();
		_pmi = new HashMap<DiscreteVariable, Double>();
		_selectedManifests = new ArrayList<DiscreteVariable>();
	}
	...
	public String getLatentName() {
		...
		for (DiscreteVariable latent : _prior.getVariables()) {
	 ...
	 */
	Set<DiscreteVariable> getLatents() {
		Set<DiscreteVariable> latents = new HashSet<DiscreteVariable>();
	 ...
	 *         variable set.
	 */
	public List<DiscreteVariable> getNaturalOrder() {
		return _order;
	}
	...
	public List<Function> getSelectedCcpds(int index) {
		...
		Map<DiscreteVariable, Function> ccpds = _ccpds.get(index);

		for (DiscreteVariable manifest : _selectedManifests) {
	 ...
	 */
	public List<DiscreteVariable> getSelectedManifests() {
	...
	public String getStateName(int index) {
		...
		List<DiscreteVariable> latents = _prior.getVariables();
		...
		for (int i = 0; i < nLatents; i++) {
			DiscreteVariable latent = latents.get(i);
	...
	void save(String filename) throws FileNotFoundException,
		...
		for (int i = 0; i < _ccpds.size(); i++) {
			...
			Map<DiscreteVariable, Function> ccpds = _ccpds.get(i);
			for (DiscreteVariable manifest : ccpds.keySet()) {
		...
		for (DiscreteVariable manifest : _order) {
			out.print(manifest.getName() + " ");
		}
		out.println();
		...
		// output PMI values
		out.println("% PMI");
		for (DiscreteVariable manifest : _pmi.keySet()) {
	...
	public String toString() {
		...
		for (int i = 0; i < nManifests; i++) {
			DiscreteVariable manifest = _selectedManifests.get(i);
