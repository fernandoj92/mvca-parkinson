...
public class CmiTree {
	...
	private class Node {
		 ...
		 */
		private Map<DiscreteVariable, Node> _children;
		 ...
		 */
		private Node() {
			_children = new HashMap<DiscreteVariable, Node>();
		}
		 ...
		 */
		private Node addChild(DiscreteVariable var, double cmi) {
			Node child = new Node();
			child._cmi = cmi;
		 ...
		 */
		private Node getChild(DiscreteVariable var) {
	 ...
	 *         whole specified path.
	 */
	boolean contains(List<DiscreteVariable> path) {
		// start from the root node
		Node current = _root;

		// traverse the path
		for (DiscreteVariable var : path) {
	 ...
	 *         path.
	 */
	List<Double> getMaxSubPathValues(List<DiscreteVariable> path) {
	...
	private void parsePreOrderString(ListIterator<String> iterator, Node node,
		...
		while (iterator.hasNext()) {
			...
			if(!token.equals("(") && !token.equals(")"))
				throw new IllegalArgumentException("'(' or ')' token was expected");
			...
			token = iterator.next();
			DiscreteVariable var = model.getNodeByName(token).getVariable();

			// delimiter
			token = iterator.next();
            if(!token.equals(","))
                throw new IllegalArgumentException("',' token was expected");
	...
	void putPathValues(List<DiscreteVariable> path, List<Double> cmiSeq) {
        if(path.size() != cmiSeq.size())
            throw new IllegalArgumentException("The path size and the size of the CMI values collection must coincide");
		...
		for (int i = 0; i < path.size(); i++) {
			DiscreteVariable var = path.get(i);
	...
	private void toPreOrderString(StringBuffer str, Node node) {
		...
		for (Map.Entry<DiscreteVariable, Node> entry : node._children.entrySet()) {
