...
public class CliqueTree extends UndirectedGraph {
	 ...
	 */
	protected Map<DiscreteVariable, CliqueNode> _variableCliques;
	...
	public CliqueTree(LTM model) {
		...
		_variableCliques = new HashMap<DiscreteVariable, CliqueNode>();

		// Construct variable clique nodes
		for (DiscreteVariable var : model.getLatVars()) {
			LinkedHashSet<DiscreteVariable> varSet = new LinkedHashSet<DiscreteVariable>();
			varSet.add(var);
		...
		for (Edge edge : model.getEdges()) {
			DiscreteVariable head = ((BeliefNode) edge.getHead()).getVariable();
			DiscreteVariable tail = ((BeliefNode) edge.getTail()).getVariable();
			LinkedHashSet<DiscreteVariable> varSet = new LinkedHashSet<DiscreteVariable>();
		...
		DiscreteVariable root = model.getRoot().getVariable();
...
//		buildCliqueTree(moralGraph, order.iterator(), bayesNet);
	...
	public CliqueTree(LTM model, DiscreteVariable[] families, DiscreteVariable[] variables) {
		this(model);
		if(variables.length == 0 || families.length == 0)
			throw new IllegalArgumentException("Invalid variables or families arrays");
		...
		for (DiscreteVariable var : families) {
	...
	public CliqueTree clone() {
		...
		if (_variableCliques != null) {
			copy._variableCliques = new HashMap<DiscreteVariable, CliqueNode>();
			for (DiscreteVariable var : _variableCliques.keySet()) {
	 ...
	 */
	public CliqueNode getVariableClique(DiscreteVariable var) {
	...
	public void copyInMsgsFrom(CliqueTree cliqueTree) {
		for (CliqueNode head : _focusedSubtree) {
			for (AbstractNode node : head.getNeighbors()) {
				...
				DiscreteVariable hVar = head.getVariables().iterator().next();
				DiscreteVariable tVar = tail.getVariables().iterator().next();
                ...
                // TODO: Dar mmás información
				if(message == null)
				    throw new InternalError("message cannot be null");
	...
	public void prune() {
		...
		Set<DiscreteVariable> vars1 = new LinkedHashSet<DiscreteVariable>();
		if (_variableCliques != null) {
			for (DiscreteVariable var : _variableCliques.keySet()) {
		...
		for (DiscreteVariable var : vars1)
			_variableCliques.remove(var);

		// Remove already-pruned things from _familyCliques
		Set<DiscreteVariable> vars2 = new LinkedHashSet<DiscreteVariable>();
		for (DiscreteVariable var : _familyCliques.keySet()) {
			if (!nodesLeft.contains(getFamilyClique(var)))
				vars2.add(var);
		}
		for (DiscreteVariable var : vars2)
