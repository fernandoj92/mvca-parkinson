...
import voltric.reasoner.CliqueNode;
import voltric.reasoner.CliqueTree;
import voltric.reasoner.CliqueTreePropagation;
import voltric.util.Function;
import voltric.util.MessagesForLocalEM;
...
//	private EmLearner _emLearner = new EmLearner();
	...
	private Map<DiscreteDataCase, CliqueTreePropagation> _ctps = new HashMap<DiscreteDataCase, CliqueTreePropagation>();

	private int _algoStep = 0;

	private ArrayList<DiscreteVariable> _ClosestVariablePair = null;

	public void setClosestVariablePair(ArrayList<DiscreteVariable> ClosestVariablePair) {
...
//	}
	 ...
	 */
	public LTM search(DiscreteDataSet dataSet) {
		DiscreteVariable[] maniVars = dataSet.getVariables();
	 ...
	 */
	public LTM search(LTM initModel, DiscreteDataSet data)
	{
		setEMLearner();
	 ...
	 */
	private LTM expand(LTM model, DiscreteDataSet data) {
		LTM bestModel = model;
		...
		while (true) 
		{
			...
			if (nextBestModel.getBICScore(data) - bestModel.getBICScore(data) > _BICthreshold) 
			{
				...
				{
					_algoStep++;
					bestModel = restrictedAdjust(bestModel, (DiscreteVariable) info[1],(DiscreteVariable) info[2], data);
					//break;
				}
				else 
				{
	 ...
	 * @return
	 */
	private LTM restrictedAdjust(LTM model, DiscreteVariable originalVar, DiscreteVariable newVar, DiscreteDataSet data)
	{
		LTM bestModel = model;
		LTM nextBestModel = null;
...
//				System.out.println("Fail\n");
	 ...
	 * @return
	 */
	public LTM refineStructureAndState(LTM model, DiscreteDataSet data, Map<DiscreteVariable, DiscreteVariable> map) {
	 ...
	 */
	private LTM oneStepExpand(LTM model, DiscreteDataSet data, Object[] info) {
		LTM bestModel = model;
		computeCtps(bestModel, data);
	 ...
	 * @return
	 */
	public LTM bestModelNI(LTM model, DiscreteDataSet data, Object[] info) {
...
//		double currentBIC = model.getBICScore(data);
		...
		for (DiscreteVariable originalVar : model.getLatVars())
		{
			// Preparation
			ArrayList<DiscreteVariable> neighbors = new ArrayList<DiscreteVariable>();
			for (AbstractNode node : model.getNode(originalVar).getNeighbors()) {
				neighbors.add(((BeliefNode) node).getVariable());
			}
			ArrayList<DiscreteVariable> children = new ArrayList<DiscreteVariable>();
...
//						.getVariable();
			...
			{		   
					DiscreteVariable vj = children.get(j);
					...
					DiscreteVariable vi = _ClosestVariablePair.get(1);
					 					
				double local_optimal = -Double.MAX_VALUE;
				for(int card = 2; card < originalVar.getCardinality()+1; card++ )
				{
						
					DiscreteVariable copyVar = new DiscreteVariable(card);
					...
					Map<DiscreteDataCase, Set<MessagesForLocalEM>> repository = new HashMap<DiscreteDataCase, Set<MessagesForLocalEM>>();
					DiscreteVariable[] families = new DiscreteVariable[] { vi, vj, copyVar };
					DiscreteVariable[] variables = new DiscreteVariable[] { copyVar };
					CliqueTreePropagation template_ = new CliqueTreePropagation(model_, families, variables);
					template_.getCliqueTree().prune();

					DiscreteVariable[] variables_enlarge = new DiscreteVariable[] { copyVar,originalVar };
					...
					for (DiscreteDataCase dataCase : data.getData()) {
...
//						ctp_.getCliqueTree().copyInMsgsFrom(ct_enlarge);
	...
	public LTM reconsiderStructure(LTM model, DiscreteDataSet data, Map<DiscreteVariable, DiscreteVariable> map)
	{
		...
		{
			DiscreteVariable parent = ((BeliefNode)(model.getNode(manifest).getParent())).getVariable();
			DiscreteVariable nearestLatentVar = map.get(manifest);
	 ...
	 * @return
	 */
	private LTM reconsiderCardinality(LTM model, DiscreteDataSet data, Map<DiscreteVariable, DiscreteVariable> map)
	{

		Map<DiscreteVariable, Integer> variablesNeedSI = new HashMap<DiscreteVariable,Integer>();
		...
		for (DiscreteVariable latVar : model.getLatVars())
		{
		 ...
		 while(true)
		 {
			...
			int nOfChildren = children.size();
			DiscreteVariable parent = null;
			if (!model.getNode(latVar).isRoot())
				parent = ((BeliefNode) model.getNode(latVar).getParent())
						.getVariable();

			// Case 2: Otherwise.
			DiscreteVariable newVar = new DiscreteVariable(latVar.getCardinality() + cardinality);
			LTM model_ = model.manipulation_VariableReplacement(latVar,
					newVar);

			// Construct repository for localEM
			Map<DiscreteDataCase, Set<MessagesForLocalEM>> repository = new HashMap<DiscreteDataCase, Set<MessagesForLocalEM>>();
			DiscreteVariable[] families = new DiscreteVariable[nOfChildren + 1];
			for (int i = 0; i < nOfChildren; i++) {
				families[i] = children.get(i);
			}
			families[nOfChildren] = newVar;
			DiscreteVariable[] variables = new DiscreteVariable[] { newVar };
			CliqueTreePropagation template_ = new CliqueTreePropagation(model_,
					families, variables);
			template_.getCliqueTree().prune();

			DiscreteVariable[] variables_enlarge = variables;
			if (parent != null)
				variables_enlarge = new DiscreteVariable[] { newVar, parent };
...
//			if (model_.getBICScore(data) - baseBICScore > CompareModelThreshold && model_.getBICScore(data) - previousBIC > CompareModelThreshold) 
		...
		for(DiscreteVariable latVar : variablesNeedSI.keySet())
		{
			 DiscreteVariable newVar = new DiscreteVariable(latVar.getCardinality() + variablesNeedSI.get(latVar));
			 ...
			 for(DiscreteVariable var : map.keySet())
			 {
	 ...
	 * @return
	 */
	public LTM bestModelSI(LTM model, DiscreteDataSet data) {
		ArrayList<LTM> sortedModelList = new ArrayList<LTM>();
		setLocalEMLearner(false);
...
//				System.out.println("Latent Variable:"+latVar.getName()+"("+latVar.getCardinality()+") got enough states!");
			...
			// Case 2: Otherwise.
			DiscreteVariable newVar = new DiscreteVariable(latVar.getCardinality() + 1);
...
//					+ model_.getLoglikelihood(data));
	...
	public LTM bestModelRestriNR(LTM model, DiscreteVariable parent, DiscreteVariable destinationVar, DiscreteDataSet data)
	{
		ArrayList<LTM> sortedModelList = new ArrayList<LTM>();
		// Note the reuse sign
...
//		computeCtps4EquiHLCM(model, equiModel, data);
		...
		{	
//			System.gc();
			DiscreteVariable alterVar = ((BeliefNode) alterNode).getVariable();
...
//				continue;
			...
			// Preparation
			ArrayList<DiscreteVariable> vAlter2Root = new ArrayList<DiscreteVariable>();
			...
			if(model.getNode(parent).getChildren().size()<= 2)
			{
				...
				{
					DiscreteVariable Child= ((BeliefNode) onlyChild).getVariable();
			...
			// Construct repository for localEM
			Map<DiscreteDataCase, Set<MessagesForLocalEM>> repository = new HashMap<DiscreteDataCase, Set<MessagesForLocalEM>>();
			DiscreteVariable[] families = new DiscreteVariable[] { alterVar };
			DiscreteVariable[] variables = new DiscreteVariable[0];
			CliqueTreePropagation template_ = new CliqueTreePropagation(model_,
					families, variables);
			template_.getCliqueTree().prune();

			DiscreteVariable[] families_enlarge = vAlter2Root
					.toArray(new DiscreteVariable[vAlter2Root.size()]);
			vAlter2Root.remove(alterVar);
			DiscreteVariable[] variables_enlarge = vAlter2Root
					.toArray(new DiscreteVariable[vAlter2Root.size()]);
			CliqueTreePropagation ctp_enlarge = new CliqueTreePropagation(
...
//					+ model_.getLoglikelihood(data));
	 ...
	 * @return
	 */
	public void computeCtps(LTM model, DiscreteDataSet data) {
		_ctps.clear();
		CliqueTreePropagation ctp = new CliqueTreePropagation(model);
	 ...
	 *         model.
	 */
	private double computeUI(LTM benchmarkModel, LTM m, DiscreteDataSet data) {
	 ...
	 * @return
	 */
	private Function computeSuffStats(DiscreteDataSet data, DiscreteVariable var1, DiscreteVariable var2) {

		LTM model = (LTM) _ctps.values().iterator().next().getBayesNet();
		ArrayList<BeliefNode> bNodesOnPath = model.computePath(var1, var2);

		Set<DiscreteVariable> queryVars = new HashSet<DiscreteVariable>();
		queryVars.add(var1);
		queryVars.add(var2);
