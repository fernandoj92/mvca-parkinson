...
public class ParallelEAST_Restricted {
	...
	private Map<DiscreteDataCase, CliqueTreePropagation> _ctps =
			new HashMap<DiscreteDataCase, CliqueTreePropagation>();
			...
			for (int j = 0; j < nOfChildren; j++) {
				...
				for (int card = 2; card < originalVar.getCardinality() + 1; card++) {
					...
					Map<DiscreteDataCase, Set<MessagesForLocalEM>> repository =
							new HashMap<DiscreteDataCase, Set<MessagesForLocalEM>>();
					...
					DiscreteVariable[] variables_enlarge =
							new DiscreteVariable[] { copyVar, originalVar };
	...
	private LTM reconsiderCardinality(LTM model, DiscreteDataSet data,
		...
		for (DiscreteVariable latVar : model.getLatVars()) {
			...
			while (true) {
				...
				DiscreteVariable newVar =
						new DiscreteVariable(latVar.getCardinality() + cardinality);
				LTM model_ =
		...
		for (DiscreteVariable latVar : variablesNeedSI.keySet()) {
			DiscreteVariable newVar =
					new DiscreteVariable(latVar.getCardinality()
	...
	public LTM bestModelRestriNR(LTM model, DiscreteVariable parent,
		...
		for (AbstractNode alterNode : model.getNode(parent).getChildren()) {
			...
			DiscreteVariable[] families_enlarge =
					vAlter2Root.toArray(new DiscreteVariable[vAlter2Root.size()]);
